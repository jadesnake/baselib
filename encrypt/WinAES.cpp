...
WinAES::WinAES( LPCTSTR lpszContainer, int nFlags )
...
m_nFlags(nFlags), m_wszContainer(NULL), m_nIndex(INVALID_INDEX)
{
    if( NULL == lpszContainer ) {
        lpszContainer = _T("Temporary - OK to Delete");
        m_nFlags |= DELETE_CONTAINER;
    }

    size_t len = _tcslen( lpszContainer ) + 1;
    assert( 0 != len );

    if( len ) {
        m_wszContainer = new TCHAR[ len ];
        assert( NULL != m_wszContainer );

        errno_t err = 0;
        err = _tcscpy_s( m_wszContainer, len, lpszContainer );
        assert( 0 == err );
    }
...
bool WinAES::AcquireContext( LPCTSTR lpszContainer )
{
